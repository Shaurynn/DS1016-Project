...
def preprocess3d(filename):
    reg = RegistrationSynQuick()
    reg.inputs.fixed_image = "data/tpl-MNI305_T1w.nii.gz"
    reg.inputs.moving_image = f"{filename}"
    reg.inputs.num_threads = 2
    reg.cmdline
    'antsRegistrationSyNQuick.sh -d 3 -f ./data/tpl-MNI305_T1w.nii.gz -r 32 -m '+f"{filename}"+' -n 2 -o ./ -p d'
    reg.run()
    print("Registration complete")
    brainextraction = BrainExtraction()
    brainextraction.inputs.dimension = 3
    brainextraction.inputs.anatomical_image ='./'+f"{filename}"+'_Warped.nii.gz'
    brainextraction.inputs.brain_template = './data/tpl-MNI305_desc-head_mask.nii.gz'
    brainextraction.inputs.brain_probability_mask ='./data/tpl-MNI305_desc-brain_mask.nii.gz'
    brainextraction.cmdline
    'antsBrainExtraction.sh -a ./'+f"{filename}"+'_Warped.nii.gz -m ./data/tpl-MNI305_desc-brain_mask.nii.gz -e ./data/tpl-MNI305_desc-head_mask.nii.gz -d 3 -s nii.gz -o ./'
    brainextraction.run()
